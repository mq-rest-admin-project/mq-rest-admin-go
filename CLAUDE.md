...
```text
~/dev/github/mq-rest-admin-go/            ← sessions ALWAYS start here
  .git/
  CLAUDE.md, cmd/, internal/, …           ← main worktree (usually `develop`)
  .worktrees/                             ← container for parallel worktrees
    issue-258-adopt-worktree-convention/  ← worktree on feature/258-...
    …
```
...
1. **Sessions always start at the project root.**
   `cd ~/dev/github/mq-rest-admin-go && claude` — never from inside
...
Your worktree is: /Users/pmoore/dev/github/mq-rest-admin-go/.worktrees/issue-<N>-<slug>/
